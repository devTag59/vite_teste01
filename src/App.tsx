  ...
  return (
    <>
      <div className='bg-gray-800 h-screen flex flex-col items-center justify-center gap-4'>
        ...
        transition-all
        duration-200' type="text" placeholder='Digite o nome da cidade' value={cidade} onKeyPress={handleKeyPress } onChange={(e) => setCidade(e.target.value)}/>
        <button className='bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded' onClick={getClima} disabled={onLoading}>Buscar Clima</button>
        </div>
      </div>
    </>
  )
